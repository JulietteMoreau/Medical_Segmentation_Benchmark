...
def limitcomma(value, limit=2):
    # truncate float to a certain limit after comma
    v = str(value).split(".")
...
def evaluate(net, dataloader, device, criterion):
    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        with torch.no_grad():
            ...
            mask_pred = net(image)
            # calculate loss
            loss = (criterion(mask_pred, mask_true.long().squeeze(1)) + dice_loss(F.softmax(mask_pred, dim=1).float(), F.one_hot(mask_true.long().squeeze(1), net.output_dim).permute(0, 3, 1, 2).float(), multiclass=True))
            
            # convert to one-hot format
            if net.output_dim == 1:
                mask_pred = (F.sigmoid(mask_pred) > 0.5).float()
                # compute the Dice score
                dice_score += dice_coeff(mask_pred, F.one_hot(mask_true.long().squeeze(1), net.output_dim).permute(0, 3, 1, 2).float(), reduce_batch_first=False)
            else:
                mask_pred = F.one_hot(mask_pred.argmax(dim=1), net.output_dim).permute(0, 3, 1, 2).float()
                # compute the Dice score, ignoring background
                dice_score += multiclass_dice_coeff(mask_pred[:, 1:, ...], F.one_hot(mask_true.long().squeeze(1), net.output_dim).permute(0, 3, 1, 2).float()[:, 1:, ...], reduce_batch_first=False)
...
def train_net(train_loader, train_ds, val_loader, outdir, patience=5, num_epoch=500,
    ...
    for epoch in range(num_epoch):
        ...
        for i, batch in enumerate(train_loader):
            ...
            loss = criterion(fake_Mask, real_Mask.long().squeeze(1)) + dice_loss(F.softmax(fake_Mask, dim=1).float(), F.one_hot(real_Mask.long().squeeze(1), net.output_dim).permute(0, 3, 1, 2).float(), multiclass=True)
        ...
        logging.info('Validation Dice score: {}'.format(val_loss))
        sys.stdout.write(
        ...
        if limitcomma(val_loss, 4) > limitcomma(last_loss,4):
            trigger_times += 1
            ...
            if trigger_times >= patience:
                ...
                plt.plot(loss_values, color='firebrick', linewidth=3)
                plt.plot(loss_val, color='navy', linestyle='--', linewidth=3)
                plt.legend(['entrainement', 'validation'], fontsize=30)
                plt.ylabel('fonction de coût', fontsize=30)
                plt.xlabel('époques', fontsize=30)
        ...
        if epoch==num_epoch-1:
            torch.save(net.state_dict(), outdir+"/checkpoints/checkpoint.pth".format(epoch + 1))
            logging.info(f'Checkpoint {epoch + 1} saved!')
